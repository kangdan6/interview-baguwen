...
1. 解耦：将不同的系统解耦开来。尤其是当你在不希望感知到下游的时候。例如我们之前做的一个语音驱动的项目，用户上传完自己的音视频文件后，因为耗时比较久，我们会把他放在一个任务队列里，排队处理完后会给用户发送邮件通知相关人员，我们上游并不关心谁想收到邮件，要给谁发邮件，所以和下游邮件发送服务之间就是通过一个消息队列解耦的，邮件服务会订阅这个消息对列，给相关人发送邮件。

2. 异步：是指将一个同步流程，利用消息队列来拆成多个步骤。（下面是我准备的一个刷亮点的方面，我们从事件驱动设计上来讨论）这个特性被广泛应用在事件驱动之中。比如我们深伪服务，判断一个图片究竟是不是伪造的，会通过读本地图片，过检脸算法，根据检脸结果过人脸识别算法，根据人脸识别算法决定走高分支还是低分支等，这些都是一个串行步骤，那我们该如何提速呢？之前有一个深伪比赛，我们为了提速就是通过多线程+消息队列解耦各个环节，比如检脸线程处理完后可以将中间结果放入一个消息队列里，然后人脸识别线程消费这个队列进行下一步操作。
